...
app.use(cors({
  origin: ['http://localhost:5173', 'http://localhost:3000', 'dpsdesk://app'],
  credentials: true,
}));
...
app.get('/', (_req, res) => {
  res.json({
    ...
    endpoints: ['/auth/login', '/devices', '/sessions', '/agents'],
  });
});
...
// Verificar si un agent específico está conectado
app.get('/agents/:posId', (req, res) => {
