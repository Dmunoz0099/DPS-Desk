...
function NetworkConfig() {
  const [cfg, setCfg] = useState({
    endpoint: 'https://api.dpsdesk.cl/v2',
    secret: 'rustdesk-shared-secret-2026',
    port: '21116',
    heartbeat: '5000',
    retries: '3',
  });
  ...
  const [lastModified, setLastModified] = useState('23.04.26 · 15:19:44');
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState(null); // {ok:bool, ms:int, msg:string}
  ...
  const runTest = () => {
    ...
    const t0 = Date.now();
    setTimeout(() => {
      const ms = 80 + Math.floor(Math.random() * 220);
      const ok = /^https?:\/\/.+/.test(cfg.endpoint) && Number(cfg.port) > 0;
      setTestResult({
        ok,
        ms,
        msg: ok
          ? `Conexión establecida con ${cfg.endpoint} en ${ms} ms · TLS 1.3 · token válido.`
          : 'No se pudo establecer conexión: revisa el endpoint y el puerto.',
      });
      setTesting(false);
  ...
  const save = () => {
    ...
    setTimeout(() => {
      ...
      setSaving(false);
      setSaveFlash('Configuración guardada · túneles reiniciados.');
